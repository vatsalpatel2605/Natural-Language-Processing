...
def entropy(p):
    ...
    s = collections.Counter(p)
    tot_ct = float(sum(p.values()))
    h = -sum( (ct/tot_ct) * math.log(ct/tot_ct) for ct in s.values())
    return h
...
def cross_entropy(p1, p2):
    ...
    xh = [(p1.get(k,0) * math.log(p2.get(k,0))) for k in p1.keys()]
    return -sum(xh)
...
def kl_divergence(p1, p2):
    ...
    kl = [(p1.get(k,0) * math.log(p2.get(k,0)/p1.get(k,0))) for k in p1.keys()]
    return -sum(kl)
