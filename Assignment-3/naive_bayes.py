    ...
    def predict(self, data, alpha=1.0):
        ...
        for classval in ["pos", "neg"]:
            for text in data.get_texts(classval):
                ...
                for w in text.get_words():
                    #if w in self.vocab:
                    sum_positive += log(self.count_positive[w] + alpha) - log((self.total_positive_words + (len(self.vocab) * alpha)))
                    sum_negative += log(self.count_negative[w] + alpha) - log((self.total_negative_words + (len(self.vocab) * alpha)))
