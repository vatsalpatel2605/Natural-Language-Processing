...
def parse_sentence(nt_rules, t_rules, words):
    ...
    for m in range(1, length):
        for n in range(0, length - m):
            chart[m][n] = {}
            for o in range(n+1, n+m+1):
                for nt in nt_rules:
                    if chart[o-n-1][n].get(nt.rhs1) and chart[n+m-o][o].get(nt.rhs2):
                        chart[m][n][nt.lhs] = ParsedPhrase(label=nt.lhs, children=[chart[o-n-1][n].get(nt.rhs1), chart[n+m-o][o].get(nt.rhs2)])
    # END TODO
